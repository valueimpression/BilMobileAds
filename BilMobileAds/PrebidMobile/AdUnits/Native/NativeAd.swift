            ...
            if let assets = json["assets"] as? [AnyObject] {
                for adObject in assets {
                    if let adObject = adObject as? [String : AnyObject]{
                        ...
                        // Data
                        if let data = adObject["data"], let type = data["type"] as? Int {
                            if type == DataAsset.description.rawValue, let text = data["value"] as? String {
                                ad.text = text;
                            }
                            if type == DataAsset.sponsored.rawValue, let text = data["value"] as? String {
                                ad.sponsoredBy = text;
                            }
                            if type == DataAsset.ctatext.rawValue, let text = data["value"] as? String {
                                ad.callToAction = text;
                            }
                        }
                        // IMG
                        if let img = adObject["img"], let type = img["type"] as? Int {
                            if type == 1, let url = img["url"] as? String { // 1 -> Icon IMG
                                ad.iconUrl = url;
                            }
                            if type == 3, let url = img["url"] as? String { // 3 -> Main IMG
                                ad.imageUrl = url;
                            }
                        }
